...
volatile uint8_t USART0_Rxbuffer[2048] = {0x0};
volatile uint32_t USART0_Rxcounter = 0;
volatile uint32_t USART0_ReceiveState = 0;

volatile uint8_t UART3_Rxbuffer[2048] = {0x0};
...
/*!
    \brief      Reset RECV BUFF of COM
    \param[in]  com          : COM0, COM1
    \retval     none
*/
void com_reset_recv(uint32_t com)
{
    if(com == COM0)
    {
        pfifo_reset(&fifo_com0);
    }else if(com == COM1)
    {
        pfifo_reset(&fifo_com1);
    }
}

/*!
    \brief      get a ch from Uart
    \param[in]  com          : COM0, COM1
    \param[out]  len         : data len of recv uart port
    \param[int]  timeout     : max timeout (ms)
    \retval     uart data buf
*/
unsigned char com_try_recv_ch(uint32_t com, int *len, int timeout)
{
    unsigned char ch = 0;
    int recv_len = 0;
    int delay = 3;
    ...
    while(1)
    {
        if(com == COM0)
        {
            recv_len = pfifo_get_ch(&fifo_com0, &ch);
        }else if(com == COM1)
        {
            recv_len = pfifo_get_ch(&fifo_com1, &ch);
        }
        if(recv_len == 1)
        {
            break;
...
void _com_init(uint32_t com)
{
    ...
    {
        index = 0;
        pfifo_init(&fifo_com0, (unsigned char *)USART0_Rxbuffer , sizeof(USART0_Rxbuffer));
    }else if (com == COM1)
    {
        index = 1;
        pfifo_init(&fifo_com1, (unsigned char *)UART3_Rxbuffer , sizeof(UART3_Rxbuffer));
    }
